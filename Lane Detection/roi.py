...
def find_roi_lane_center(roi, roi_center, frame = None):
    ...
    # draw the lanes if frame is provided
    if frame is not None:
        if is_right_lane_detected:
            cv.circle(frame, (right_lane_x, roi_center_y + constant.ROI_HEIGHT_LOWER_BOUND), 4, constant.GREEN_COLOR, 2)
        if is_left_lane_detected:
            cv.circle(frame, (left_lane_x, roi_center_y + constant.ROI_HEIGHT_LOWER_BOUND), 4, constant.GREEN_COLOR, 2)
    else:
        print('Frame is not provided')
